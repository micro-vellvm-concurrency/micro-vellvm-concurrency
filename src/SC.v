...
Definition handle_mem {E C} `{PickC -< C} :
  ThreadAnnotE MemoryE ~> Monads.stateT SCMem (ctree E (B01 +' C)) :=
  fun _ e s =>
    let m := s in
    match e with
    | ThreadAnnot _ e =>
...
Section InterpSC.
  ...
  Definition E_trigger : E ~> Monads.stateT SCMem (ctree E (B01 +' PickC +' C)) :=
   fun _ e m => r <- trigger e ;; ret (m, r).

  Definition interp_mem_h : ThreadAnnotE MemoryE +' E ~>
    Monads.stateT SCMem (ctree E (B01 +' PickC +' C)) :=
    case_ handle_mem E_trigger.

  Definition interp_mem : ctree (ThreadAnnotE MemoryE +' E) (B01 +' C) ~>
    Monads.stateT SCMem (ctree E (B01 +' PickC +' C)) :=
    interp interp_mem_h.
End InterpSC.
