...
Section InterpVar.
  ...
  Definition E_trigger : E ~> Monads.stateT var_state (ctree E (B01 +' C)) :=
    fun _ e s => x <- trigger e;; Ret (s, x).

  Definition interp_var_h :
    VarE +' E ~>
    Monads.stateT var_state (ctree E (B01 +' C)) :=
    fun _ e s =>
      match e with
  ...
  Definition interp_var :
      ctree (VarE +' E) (B01 +' C) ~> Monads.stateT var_state (ctree E (B01 +' C)) :=
