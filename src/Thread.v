...
Context (fns : FnMap.t (list Arg -> ctree (@ThreadE (list Arg) +' TE +' E) (B01 +' C) A)).
...
Definition build_thread_ctree (f init cleanup : function_id) (args : list Arg) :
  option (ctree (ThreadE +' TE +' E) (B01 +' C) A) :=
...
Definition step_thread_event {X}
    atomic nextTid tid
    (l : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' C) A))
    (e : @ThreadE (list Arg) X) (k : X -> ctree (ThreadE +' TE +' E) (B01 +' C) A) :
    ctree F (B01 +' SchedC +' C) (bool * thread_id * TMap.t _) :=
  match e, k with
...
Notation step_thread_ atomic nextTid tid l t :=
  ...
  | BrSF c k =>
      brS c (fun x =>
        Ret (atomic, nextTid, TMap.add tid (k x) l))
  | BrDF c k =>
      brD c (fun x => step_thread atomic nextTid tid l (k x))
  | VisF e k =>
    match e with
    | inl1 e =>
      step_thread_event atomic nextTid tid l e k
...
Definition interleave
  tidExec nextTid (l : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' C) A)) :
  ctree F (B01 +' SchedC +' C) nat :=
  CTree.iter (fun '(tidExec, nextTid, l) =>
    ...
    | None => brD (Sched (TMap.keys l)) (fun tid => Ret (tid, false))
    end;;
...
Notation interleave_ atomic nextTid l :=
  ...
  tidExec <- branchD (Sched (TMap.keys l));;
  match TMap.find tidExec l with
  | Some t =>
      CTree.bind (step_thread atomic nextTid tidExec l t)
       (fun '(atomic, nextTid, l) =>
...
Definition step_interleave
  atomic nextTid tidExec (l : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' C) A)) t :
    ctree F (B01 +' SchedC +' C) nat :=
...
Proof.
  ...
  - rewrite bind_bind. destruct tidExec.
    + rewrite !bind_ret_l.
      destruct (TMap.find t l).
      * rewrite !bind_bind. unfold step_interleave. cbn.
        upto_bind_eq. destruct x1 as ((? & ?) & ?).
...
Definition interleave_top
  (main : ctree (ThreadE +' TE +' E) (B01 +' C) A) :
  ctree F (B01 +' SchedC +' C) nat :=
  interleave None 2%nat (TMap.singleton 1%nat main).

Definition handle_thread (tid : thread_id) : @ThreadE Arg ~> Monads.stateT nat (ctree F (B01 +' SchedC +' C)) :=
  fun _ e s =>
    match e with
...
Definition interp_thread_h tid :
  ThreadE +' TE +' E ~> Monads.stateT nat (ctree F (B01 +' SchedC +' C)) :=
  fun _ e =>
    match e with
...
Definition interp_thread
  tidExec nextTid (t : (ctree (ThreadE +' TE +' E) (B01 +' C) A)) :
  ctree F (B01 +' SchedC +' C) (thread_id * A) :=
...
CoFixpoint merge_threads {E C}
  (t1 t2 : ctree E (B01 +' SchedC +' C) unit) :
  ctree E (B01 +' SchedC +' C) unit :=
  match observe t1, observe t2 with
  | RetF _, _ => t2
  | _, RetF _ => t1
  | VisF e k, _ => Vis e (fun x => merge_threads (k x) t2)
  | _, VisF e k => Vis e (fun x => merge_threads t1 (k x))
  | BrF b1 c1 k1, BrF b2 c2 k2 =>
    match c1, c2 with
    | inr1 (inl1 e1), inr1 (inl1 e2) =>
      match e1, k1, e2, k2 with
      | Sched l1, k1, Sched l2, k2 =>
        brD (Sched (l1 ++ l2)) (fun tid =>
          merge_threads (k1 tid) (k2 tid)
        )
      end
    | _, inr1 (inl1 _) => br b1 c1 (fun x => merge_threads (k1 x) t2)
    | _, _ => br b2 c2 (fun x => merge_threads t1 (k2 x))
    end
  end.
