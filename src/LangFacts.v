...
#[local] Notation interp_var globals t locals := (interp_state (M := ctree E (B01 +' void1')) (interp_var_h globals) (t : ctree (VarE +' E) (B01 +' void1') _) locals).

Lemma interp_repr_atom_pure :
  forall a globals locals,
  pure_finite (interp_var globals (repr_atom a) locals) \/
  interp_var globals (repr_atom a) locals ~ (stuckD : ctree E (B01 +' void1') (var_state * value)).
Proof.
...
Lemma interp_repr_atom_case :
  forall a globals locals,
  (exists v : value, forall E C `{exceptE string -< E}, interp_var (C := C) globals _ (repr_atom a) locals ~ (Ret (locals, v) : ctree E (B01 +' C) (var_state * value))).
Proof.
...
Lemma interp_repr_exp_case :
  ...
  (exists v : value, forall E C `{exceptE string -< E}, interp_var (C := C) globals _ (repr_exp e) locals ~ (Ret (locals, v) : ctree E (B01 +' C) (var_state * value))).
Proof.
