...
Definition repr_ecfg (e : ecfg) : ctree E0 (B01 +' void1') value :=
  r <- repr_block (fst e);;
  match r with
...
Proof.
  intros. cbn. rewrite !bind_bind. upto_bind_eq. upto_bind_eq.
...
Proof.
  intros. cbn. rewrite !bind_bind. rewrite !bind_ret_l. upto_bind_eq.
  destruct x1. cbn. unfold repr_ocfg. rewrite unfold_iter.
  rewrite bind_bind.
  destruct (find_block c b) eqn:?.
...
Proof.
  intros. cbn -[repr_ecfg].
  upto_bind_eq. upto_bind_eq.
  now rewrite bind_ret_r.
Qed.

Definition repr_codes (c : codes) : TMap.t (ctree E0 (B01 +' void1') value) :=
  TMap.map (fun ll => repr_task ll) c.

Lemma find_interp0_l {A} : forall (p : TMap.t (ctree E0 (B01 +' void1') A)) g vars,
...
Lemma is_empty_interp0_l {A} : forall (p : TMap.t (ctree E0 (B01 +' void1') A)) g vars,
  (forall tid, TMap.In tid p -> TMap.In tid vars) ->
...
Definition step_instr (fns : list definition) (nextTId : nat) (p : codes) (g : var_state) (vars : TMap.t var_state) (tidExec : nat) :
  ctree E2 (B01 +' SchedC +' void1') (nat * codes * (TMap.t var_state)) :=
  ...
  | Some (mk_block bid [] term, c, tk) =>
    '(nextTId, v) <- interp_thread (F := E2) tidExec nextTId
      (interp0 (repr_term (E := E0) (B := B01 +' void1') term) g (TMap.find_or tidExec StringMap.empty vars));;
...
Ltac ctsimpl_term t :=
  ...
  | CTree.bind (Br ?b ?c ?k) ?k' => rewrite (bind_br b c k k')
  ...
  | interp_state ?h (Br ?b ?c ?k) ?st => rewrite (interp_state_br h b c k st)
...
Ltac ctsimpl_one :=
  match goal with
  | |- ?t ~ ?u => ctsimpl_term t || ctsimpl_term u
  end.
...
Theorem step_instr_op : forall (fns : list definition) nextTId tidExec (p : codes) g vars bid reg e code term c tk t,
  ...
  TMap.find_or tidExec stuckD (interp0_l (repr_codes p) g vars) ≅ t ->
...
Lemma repr_definitions_map : forall fns,
  StringMap.Equal
    ...
    (StringMap.map (repr_cfg (E := E0) (B := B01 +' void1')) (StringMapFacts.of_list fns)).
Proof.
...
Lemma repr_ecfg_repr_cfg : forall cfg args,
  repr_ecfg (init_ecfg cfg args) ~ (wrap_thread (repr_cfg cfg) args : ctree E0 (B01 +' void1') value).
Proof.
  ...
  - unfold repr_instr'.
    ...
    + cbn -[append String.nat2string10].
      ...
      upto_bind. apply IHargs. reflexivity.
...
Proof.
  ...
  upto_bind. reflexivity. intros _. apply H2.
Qed.
...
Lemma init_task_build_thread_ctree : forall fns g fn init cleanup v',
  option_rel (sbisim eq)
    (option_map (fun tk => interp_state (MM := @Monad_ctree E1 (B01 +' void1')) (interp_var_h g)
...
Lemma interp0_l_In {A} : forall (p : TMap.t (ctree E0 (B01 +' void1') A)) g vars,
  (forall tid, TMap.In tid p -> TMap.In tid vars) ->
  forall tid,
...
Proof.
  ...
  apply sb_brD_id. intros tidExec. rewrite bind_ret_l.
