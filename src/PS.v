...
Definition handle_mem {E C} `{PSMemC -< C} `{SchedC -< C} `{PickC -< C} `{PSExceptC -< C} `{DebugLogC -< C} :
  ThreadAnnotE MemoryE ~> Monads.stateT PSMem (ctree E (B01 +' C)) :=
  fun _ e s =>
    ...
    match e with ThreadAnnot tid e =>
      ...
      | Read o a =>
        if is_not_atomic o then
          ...
          '(_, date) <- branchD (PSUpdateView s tid a PSRead);;
          r <- lift_either (mem_read o s tid a date);;
          v <- freeze (snd r);;
          Step (ret (fst (fst r), v))
      | Write o a v =>
        '(ts_from, ts_to) <- branchD (PSUpdateView s tid a (PSFulfill v));;
        s <- lift_either (mem_write o s tid (mkMsg a v ts_from ts_to NatMap.empty));;
        Step (ret (s, tt))
      | ReadWrite o a f =>
        '(ts_from, ts_to) <- branchD (PSUpdateView s tid a PSFulfillUpdate);;
...
Section InterpPS.
  ...
  Definition E_trigger : E ~> Monads.stateT PSMem (ctree F (B01 +' D)) :=
    fun R e m => r <- trigger e ;; ret (m, r).

  Definition interp_mem_h : (ThreadAnnotE MemoryE +' E) ~>
    Monads.stateT PSMem (ctree F (B01 +' D)) :=
    case_ handle_mem E_trigger.

  Definition C_branch b : C ~> Monads.stateT PSMem (ctree F D) := fun _ x m => c <- branch b x;; ret (m, c).
...
Section InterpView.
  ...
  Context {S : Type}.
  Variable handler : PSMemC ~> Monads.stateT S (ctree E (B01 +' B)).

  Definition B_branch' b : B ~> Monads.stateT S (ctree E (B01 +' B)) := fun _ x m => c <- branch b x;; ret (m, c).
  Definition B01_branch b : B01 ~> Monads.stateT S (ctree E (B01 +' B)) := fun _ x m => c <- branch b x;; ret (m, c).

  Definition interp_view_h b : B01 +' (PSMemC +' B) ~> Monads.stateT S (ctree E (B01 +' B)) :=
    case_ (B01_branch b) (case_ handler (B_branch' b)).

  Definition interp_view : ctree E (B01 +' PSMemC +' B) ~> Monads.stateT S (ctree E (B01 +' B)) :=
...
Proof.
  ...
  - destruct p as ((? & ?) & ?).
    inv H. cbn. rewrite Heqo. split; reflexivity.
  - discriminate.
Qed.
...
Lemma Lmem_read_na {E C D} `{B0 -< C} `{B0 -< D} `{PickC -< C} `{PickC -< D} : forall sc ps tid a,
  Lmem sc ps ->
  (freeze (SC.mem_read sc a) : ctree E C value) ≲ (freeze (mem_read_na ps tid a) : ctree E D value).
Proof.
  intros. rewrite <- (proj1 (proj2 H3)). unfold mem_read_na.
  destruct (last_msg a (globalMem ps)) eqn:?; cbn.
  - destruct (Qeq_bool _ _) eqn:?; cbn.
    + now apply ssim_ret.
    + apply ssim_brD_r with (x := val m). now apply ssim_ret.
  - apply ssim_brD_id. intros. now apply ssim_ret.
Qed.

Theorem ss_sc_ps {E C} : forall (t : ctree (ThreadAnnotE MemoryE +' E) (B01 +' C) unit)
  sc mem,
  Lmem sc mem ->
    ...
    + (* Read *)
      ...
      destruct (is_not_atomic o) eqn:?. {
       eapply ssim_clo_bind_gen with (R0 := eq). 2: apply Lmem_read_na; auto.
        ...
        intros ?? <-. apply ssim_step; [| constructor; etrans]. apply ssim_ret. now constructor.
      }
      rewrite bind_branch. apply ssim_brD_r with (x := latest st' k PSRead).
      ...
      { cbn. rewrite <- last_msg_max_view. rewrite (proj1 (proj2 H0)).
        ...
        - destruct (is_racy _). eapply ssim_brD_r. now apply ssim_ret.
          now apply ssim_ret.
        - destruct (is_racy _); apply ssim_brD_id; intros; now apply ssim_ret.
      }
      intros ?? <-. apply ssim_step; [| constructor; etrans].
      apply ssim_ret. constructor. split; auto.
      apply H0.
    + (* Write *)
      cbn. rewrite bind_branch. apply ssim_brD_r with (x := latest st' k (PSFulfill v)).
      ...
      apply ssim_step; [| constructor; etrans].
      apply ssim_ret. constructor. split; auto.
      eapply Lmem_write. 1: apply H0.
      erewrite mem_write_latest; try reflexivity. apply H0.
    + (* ReadWrite *)
      cbn.
      rewrite bind_branch. apply ssim_brD_r with (x := latest st' k PSFulfillUpdate).
      ...
      { cbn. rewrite <- last_msg_max_view. rewrite (proj1 (proj2 H0)).
        ...
        { destruct (is_racy _); cbn.
          eapply ssim_brD_r. now apply ssim_ret. now apply ssim_ret. }
        rewrite max_view_last_msg, Heqo0. cbn. rewrite Tauto.if_same.
        unfold branchD. apply ssim_brD_id. intros. now apply ssim_ret.
      }
