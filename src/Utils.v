...
Lemma brD1_guard : forall {E B X} `{B1 -< B} (k : unit -> ctree E B X),
  brD branch1 k ≅ Guard (k tt).
Proof.
  intros. step. constructor. now intros [].
Qed.

Lemma st_guard : forall E C `{B0 -< C} `{B1 -< C} A t t' (R : relation (ctree E C A)), st eq R t t' -> st eq R t (Guard t').
Proof.
  intros. rewrite sb_guard. apply H1.
Qed.
...
Proof.
  ...
  - apply step_sb'_br_id; auto. constructor; etrans.
Qed.
