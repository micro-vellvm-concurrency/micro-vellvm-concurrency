...
Definition handle_mem {E C} `{TSOFlushC -< C} `{PickC -< C} :
  ThreadAnnotE MemoryE ~> Monads.stateT TSOMem (ctree E (B01 +' C)) :=
  fun _ e s =>
    '(s, r) <- match e with ThreadAnnot tid e =>
      match e in (MemoryE T) return (ctree E (B01 +' C) (TSOMem * T)) with
      | Read _ a =>
...
Section InterpTSO.
  ...
  Definition E_trigger : E ~> Monads.stateT TSOMem (ctree E (B01 +' TSOFlushC +' PickC +' C)) :=
    fun _ e m => r <- trigger e ;; ret (m, r).

  Definition interp_mem_h : ThreadAnnotE MemoryE +' E ~>
    Monads.stateT TSOMem (ctree E (B01 +' TSOFlushC +' PickC +' C)) :=
    case_ handle_mem E_trigger.

  Definition interp_mem :
    ctree (ThreadAnnotE MemoryE +' E) (B01 +' C) ~>
    Monads.stateT TSOMem (ctree E (B01 +' TSOFlushC +' PickC +' C)) :=
    interp interp_mem_h.
...
Theorem ss_sc_tso {E C} : forall (t : ctree (ThreadAnnotE MemoryE +' E) (B01 +' C) unit)
  m m',
  Lmem m m' ->
  (SC.interp_mem _ t m)
    (≲ lift_val_rel Lmem')
...
Proof.
  ...
  - (* Read *)
    ...
    + rewrite !bind_ret_l, bind_step.
      ...
      apply ssim_brD_r with (x := []).
      apply ssim_ret. constructor. split; auto.
    + rewrite bind_bind, !bind_branch. apply ssim_brD_id. intros.
      rewrite bind_step. apply ssim_step; [| constructor; etrans].
      rewrite bind_ret_l, bind_branch. apply ssim_brD_r with (x := []).
      apply ssim_ret. constructor. split; auto.
  - (* Write *)
    rewrite bind_step. apply ssim_step; [| constructor; etrans].
    rewrite bind_ret_l, bind_branch.
    apply ssim_brD_r with (x := [tid]).
    apply ssim_ret. constructor. split; auto.
  ...
  - (* ReadWrite *)
    ...
    + rewrite !bind_branch. apply ssim_brD_id. intros.
      rewrite bind_step. apply ssim_step; [| constructor; etrans].
      rewrite bind_ret_l, bind_branch.
      apply ssim_brD_r with (x := [tid]).
      apply ssim_ret. constructor. split; auto.
...
Section InterpFlush.
  ...
  Context {S : Type}.
  Variable handler : TSOFlushC ~> Monads.stateT S (ctree E (B01 +' B)).
  ...
  Definition interp_flush_h b : B01 +' (TSOFlushC +' B) ~> Monads.stateT S (ctree E (B01 +' B)) :=
    case_ (B01_branch b) (case_ handler (B_branch' b)).

  Definition interp_flush : ctree E (B01 +' TSOFlushC +' B) ~> Monads.stateT S (ctree E (B01 +' B)) :=
