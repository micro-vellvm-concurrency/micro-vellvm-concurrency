...
Proof.
  cbn. intros fns fns' Hfns get_val ? <- atomic ? <- nextTid ? <- tidExec ? <- tl tl' Htl t ? <-.
...
Definition Rstep_equ {E B X} : relation (bool * nat * _) :=
  (eq * eq * TMap.Equal1 (@equ E (B01 +' B) X X eq))%signature.

Definition Rstep_sb {E B X} : relation (bool * nat * _) :=
  (eq * eq * TMap.Equal1 (@sbisim E E (B01 +' B) (B01 +' B) X X _ _ eq))%signature.

Definition Rstep_ss {E B X} : relation (bool * nat * _) :=
  (eq * eq * TMap.Equal1 (@ssim E E (B01 +' B) (B01 +' B) X X _ _ eq))%signature.
...
Context (fns : FnMap.t (list Arg -> ctree (@ThreadE (list Arg) +' TE +' E) (B01 +' B) X))
  (get_val : X -> Arg) (atomic : bool) (nextTId : nat) 
  (tidExec : TMap.key) (pl : TMap.t (ctree (@ThreadE (list Arg) +' TE +' E) (B01 +' B) X)).

Lemma step_thread_brD_inv {Y} : forall p
  (c : (B01 +' SchedC +' B) Y) k x,
  step_thread (F := F) fns get_val atomic nextTId tidExec pl p ≅ BrD c k ->
  exists c0 k0, p ≅ BrD c0 k0 /\
    c = subevent _ c0 /\
...
Proof.
  ...
  - destruct vis; inv_equ. exists c0, k0.
    now rewrite ctree_eta, Heqc0, EQ.
Qed.

Lemma trans_step_thread_obs {Y} : forall
  (p : ctree (ThreadE +' TE +' E) (B01 +' B) X) t' (e : E Y) x,
...
Proof.
  ...
  - specialize (IHtrans_ Heqot' Heql _ eq_refl). destruct IHtrans_ as (t'' & TR & EQ).
    exists t''. split; auto. rewrite unfold_step_thread, <- Heqop. eapply trans_brD. 2: reflexivity. etrans.
  - inv Heql.
...
Lemma trans_step_thread_obs_annot {Y} : forall
  (p : ctree (ThreadE +' TE +' E) (B01 +' B) X) t' (e : TE Y) x,
  trans (obs (subevent _ e) x) p t' ->
  exists t'',
...
Lemma trans_step_thread_spawn : forall
  (p : ctree (ThreadE +' TE +' E) (B01 +' B) X) t' f init cleanup (a : list Arg),
  ...
  exists t'', trans (val (atomic, S nextTId,
    (TMap.add tidExec t''
      ...
      (step_thread (F := F) fns get_val atomic nextTId tidExec pl p) stuckD /\
    t'' ≅ t'.
Proof.
  intros. do 3 red in H.
  remember (observe p) as op. remember (observe t') as ot'. remember (obs _ _).
...
Lemma trans_step_thread_setAtomic : forall
  (p : ctree (ThreadE +' TE +' E) (B01 +' B) X) t' a,
  trans (obs (subevent _ (SetAtomic a)) tt) p t' ->
  exists t'', trans (val (a, nextTId, (TMap.add tidExec t'' pl)))
    (step_thread (Arg := Arg) (F := F) fns get_val atomic nextTId tidExec pl p) stuckD /\
    t'' ≅ t'.
Proof.
...
Lemma trans_step_thread_tau : forall
  (p : ctree (ThreadE +' TE +' E) (B01 +' B) X) t',
  trans tau p t' ->
  exists t'', trans tau
    (step_thread (Arg := Arg) (F := F) fns get_val atomic nextTId tidExec pl p)
    (Ret (atomic, nextTId, (TMap.add tidExec t'' pl))) /\
    t'' ≅ t'.
Proof.
  intros. do 3 red in H. remember (observe p) as op. remember (observe t') as ot'. remember tau.
  revert p Heqop. induction H; intros.
  - specialize (IHtrans_ Heqot' Heql _ eq_refl). destruct IHtrans_ as (t'' & TR & EQ).
    exists t''. split; auto. rewrite unfold_step_thread, <- Heqop. etrans.
  - exists (k x).
...
Lemma trans_step_thread_val : forall
  (p : ctree (ThreadE +' TE +' E) (B01 +' B) X) t' (v : X),
  trans (val v) p t' ->
  ...
  - destruct e0.
    ...
    + destruct s; inv_equ.
      ...
      * left. exists e0, k0. split; auto. rewrite ctree_eta, Heqc. split; auto.
      intros. now rewrite EQ.
  - destruct vis; inv_equ.
Qed.

Lemma step_thread_brS_inv {Y} : forall p (c : (B01 +' SchedC +' B) Y) k,
  step_thread (Arg := Arg) (F := F) fns get_val atomic nextTId tidExec pl p ≅ BrS c k ->
  exists c0 k0, p ≅ BrS c0 k0 /\ c = subevent _ c0 /\
    (forall x, k x ≅ Ret (atomic, nextTId, (TMap.add tidExec (k0 x) pl))).
Proof.
  intros.
  rewrite unfold_step_thread in H.
  destruct (observe p) eqn:?.
  - inv_equ.
  - destruct e as [? | []]; inv_equ. destruct t; inv_equ.
  - destruct vis; inv_equ.
    exists c0, k0. split.
    + now rewrite ctree_eta, Heqc0.
    + split; auto. intros. now rewrite EQ.
Qed.
...
Proof.
  ...
  - destruct e0 as [| []].
    + unfold step_thread_event. destruct t.
      * constructor.
        split; auto.
        apply NatMapFacts.add_m1; auto. apply add_thread_equ; auto.
      * constructor. cbn.
        split; auto.
        apply NatMapFacts.add_m1; auto.
    + constructor. intros.
...
Proof.
  ...
  - cbn in H.
    ...
    constructor. intros _. now apply CH.
  - constructor. intros _. apply CH; auto.
Qed.
...
Proof.
  ...
  cbn in H0. destruct H0 as ((-> & ->) & ?).
  step. constructor. intros _. now apply interleave_equ.
Qed.
...
#[global] Instance step_thread_ssim :
  Proper (FnMap.Equal1 (eq ==> ssim eq) ==> eq ==> eq ==> eq ==> eq ==>
      TMap.Equal1 (@ssim (ThreadE +' TE +' E) _ (B01 +' B) (B01 +' B) X X _ _ eq) ==>
      ssim eq ==>
...
Proof.
  ...
  - symmetry in EQ. apply step_thread_brD_inv with (x := x) in EQ as (?c & ?k & EQp & EQk).
    subs. apply ss_brD_l_inv with (x := x) in SIM.
    apply IHTR in SIM; auto. now rewrite <- ctree_eta.
  - symmetry in EQ. apply step_thread_brS_inv in EQ as (c0 & k0 & EQp & EQc & EQk).
    subs. apply ss_brS_l_inv with (x := x) in SIM as (l' & u' & TR & SIM & <-).
  ...
  - symmetry in EQ. destruct r as ((? & ?) & ?).
    ...
    + rewrite EQp in SIM. apply ss_ret_l_inv in SIM as (l' & u' & TR & SIM & <-).
      ...
      { rewrite (ctree_eta t'), <- Heqot'. step. apply is_stuck_sb; apply Br0_is_stuck. }
...
Proof.
  ...
  intros [] [=]. rewrite <- H1, <- H2. clear t b H1 H2.
...
#[global] Instance step_thread_sbisim :
  Proper (FnMap.Equal1 (eq ==> sbisim eq) ==> eq ==> eq ==> eq ==> eq ==>
      TMap.Equal1 (@sbisim (ThreadE +' TE +' E) _ (B01 +' B) (B01 +' B) X X _ _ eq) ==>
      sbisim eq ==>
...
Lemma step_thread_brD {Y} : forall
  fns get_val atomic nextTId (pl : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' B) X))
  tidExec (c : (B01 +' B) Y) k,
  step_thread (F := F) fns get_val atomic nextTId tidExec pl (BrD c k) ≅
    brD c (fun x => step_thread (Arg := Arg) (F := F) fns get_val atomic nextTId tidExec pl (k x)).
Proof.
...
Lemma step_interleave_setAtomic : forall
  fns get_val atomic nextTId (pl : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' B) X)) tidExec atomic1 k,
...
Lemma step_interleave_spawn : forall
  fns get_val atomic nextTId (pl : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' B) X)) tidExec fn init cleanup arg k,
...
Lemma step_thread_bind_pure : forall
  fns get_val atomic nextTId (pl : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' B) X)) tidExec t k,
  pure_finite t ->
...
Proof.
  ...
  - rewrite EQ at 2. subs. rewrite FoldStateT.interp_state_br. cbn -[Rstep_sb].
    ...
    step. apply step_sb_brD_id. intro.
    rewrite bind_guard. symmetry. apply step_sb_brD_l; auto. intros _. symmetry.
    unfold bt, comp. red. rewrite <- ((gfp_fp (sb _) _ _)). apply H.
Qed.
...
Lemma step_thread_is_stuck : forall
  fns get_val atomic nextTId (pl : TMap.t (ctree (ThreadE +' TE +' E) (B01 +' B) X)) tidExec t,
  is_stuck t ->
...
Proof.
  ...
  - apply step_thread_brD_inv with (x := x) in H1 as (? & ? & ? & -> & ?).
    ...
    { intros ???. eapply trans_brD in H3. rewrite <- H1 in H3. now apply H in H3. reflexivity. }
    now rewrite <- ctree_eta, H2.
  - apply step_thread_brS_inv in H1 as (? & ? & ? & -> & ?).
