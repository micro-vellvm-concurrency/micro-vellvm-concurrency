...
Definition Bsched := B01 +' SchedC +' void1'.
Definition B_SC := B01 +' PickC +' SchedC +' void1'.
Definition B_TSO := B01 +' TSOFlushC +' PickC +' SchedC +' void1'.
Definition B_PS := B01 +' PSMemC +' PickC +' SchedC +' DebugLogC +' ExceptC PSMem +' void1'.

Notation repr_code := (repr_code (E := E0) (B := B01 +' void1')).

Definition repr_codes tl := TMap.map repr_code tl.

Definition repr_ll (p : toplevel) := repr_toplevel (E := E0) (B:= B01 +' void1') p "main".

Definition wrap_threads {A} (fns : FnMap.t (ctree E0 (B01 +' void1') A)) :=
  FnMap.map wrap_thread fns.

Definition interp0 {A} (t : ctree E0 (B01 +' void1') A) g v :
    ctree E1 (B01 +' void1') (var_state * A) :=
  interp_var g _ t v.

Definition interp0_l {A} (tl : TMap.t (ctree E0 (B01 +' void1') A)) g (v : TMap.t var_state) :
    TMap.t (ctree E1 (B01 +' void1') (var_state * A)) :=
  NatMapFacts.map2 (fun (t : option _) (v : option _) =>
    match t, v with | Some t, Some v => Some (interp0 t g v) | _, _ => None end) tl v.

Definition interp0_l' {A} (fns : FnMap.t (list value -> ctree E0 (B01 +' void1') A)) g (v : FnMap.t var_state) :
    FnMap.t (list value -> ctree E1 (B01 +' void1') (var_state * A)) :=
...
Definition interp0_top {A} (tl : FnMap.t (ctree E0 (B01 +' void1') A)) g :
    FnMap.t (list value -> ctree E1 (B01 +' void1') (var_state * A)) :=
...
Definition interp1 (fns : FnMap.t (list value -> ctree E1 (B01 +' void1') (var_state * value)))
  (nextTId : thread_id) (tl : TMap.t (ctree E1 (B01 +' void1') (var_state * value))) :
    ctree E2 Bsched nat :=
  interleave (TE := MemoryE) fns snd None nextTId tl.

Definition interp1_top (fns : FnMap.t (list value -> ctree E1 (B01 +' void1') (var_state * value))) :
    ctree E2 Bsched nat :=
  interleave_top fns snd (FnMap.find_or "main" (fun _ => stuckD) fns []).

Definition interp2 {A} (t : ctree E2 Bsched A) addr :
    ctree E3 Bsched (nat * A) :=
  interp_alloc _ t addr (*1%nat*).

Definition interp_common (fns : FnMap.t (ctree E0 (B01 +' void1') value))
  (nextTId : thread_id) (tl : TMap.t (ctree E0 (B01 +' void1') value)) g v addr :
    ctree E3 Bsched (nat * nat) :=
  interp2 (interp1 (interp0_top fns g) nextTId (interp0_l tl g v)) addr.

Definition interp_common_top (tl : FnMap.t (ctree E0 (B01 +' void1') value)) g addr :
    ctree E3 Bsched (nat * nat) :=
...
Definition interp_sc (fns : FnMap.t (ctree E0 (B01 +' void1') value))
  (nextTId : thread_id) (tl : TMap.t (ctree E0 (B01 +' void1') value)) g v addr sc :
    ctree E4 B_SC (SCMem * (nat * nat)) :=
  interp3_sc (interp_common fns nextTId tl g v addr) sc.

Definition interp_sc_top (tl : FnMap.t (ctree E0 (B01 +' void1') value)) g addr sc :
    ctree E4 B_SC (SCMem * (nat * nat)) :=
...
Definition interp_tso (fns : FnMap.t (ctree E0 (B01 +' void1') value))
  (nextTId : thread_id) (tl : TMap.t (ctree E0 (B01 +' void1') value)) g v addr tso :
    ctree E4 B_TSO (TSOMem * (nat * nat)) :=
  interp3_tso (interp_common fns nextTId tl g v addr) tso.

Definition interp_tso_top (tl : FnMap.t (ctree E0 (B01 +' void1') value)) g addr tso :
    ctree E4 B_TSO (TSOMem * (nat * nat)) :=
...
Definition interp_ps (fns : FnMap.t (ctree E0 (B01 +' void1') value))
  (nextTId : thread_id) (tl : TMap.t (ctree E0 (B01 +' void1') value)) g v addr ps :
    ctree E4 B_PS (PSMem * (nat * nat)) :=
  interp3_ps (interp_common fns nextTId tl g v addr) ps.

Definition interp_ps_top (tl : FnMap.t (ctree E0 (B01 +' void1') value)) g addr ps :
    ctree E4 B_PS (PSMem * (nat * nat)) :=
...
#[global] Instance is_simple_handle_var :
  forall {E C X} g (e : VarE X) st, is_simple (@handle_var g E (B01 +' C) X e st).
Proof.
...
#[global] Instance is_simple_handle_alloc_simple :
  forall {E C X} (e : AllocE X) n, is_simple (@handle_alloc_simple E (B01 +' C) X e n).
Proof.
  intros. destruct e; typeclasses eauto.
Qed.
