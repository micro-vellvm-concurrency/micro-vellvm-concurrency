...
let rec collect_sc t depth =
  ...
  | BrF (_b, c, k) ->
    match c with
    | (* B0 *) Coq_inl (Coq_inl _) -> MemSet.empty
    | (* B1 *) Coq_inl (Coq_inr _) -> collect_sc (k (Obj.magic ())) depth
    | (* Pick *) Coq_inr (Coq_inl _) -> (*print_string "pick";*) collect_sc (k (Obj.magic (z_of_int (-50)))) (depth + 1)
...
let rec collect_tso t depth =
  ...
  | BrF (_b, c, k) ->
    match c with
    | (* B0 *) Coq_inl (Coq_inl _) -> MemSet.empty
    | (* B1 *) Coq_inl (Coq_inr _) -> collect_tso (k (Obj.magic ())) depth
    | (* Flush *) Coq_inr (Coq_inl s) ->
    ...
    | (* Pick *) Coq_inr (Coq_inr (Coq_inl _)) -> collect_tso (k (Obj.magic (z_of_int (-50)))) (depth + 1)
    | (* Sched *) Coq_inr (Coq_inr (Coq_inr (Coq_inl tl))) ->
        List.fold_left (fun memset tid ->
          MemSet.union
            memset
            (collect_tso (k (Obj.magic tid)) (depth + 1))
        ) MemSet.empty tl
    | (* void1 *) Coq_inr (Coq_inr (Coq_inr (Coq_inr _))) -> MemSet.empty
  in
  ret
;;
...
let rec collect_ps t depth =
  ...
  | BrF (_b, c, k) ->
    match c with
    | (* B0 *) Coq_inl (Coq_inl _) -> MemSet.empty
    | (* B1 *) Coq_inl (Coq_inr _) -> collect_ps (k (Obj.magic ())) depth
    | (* UpdateView *) Coq_inr (Coq_inl (mem, tid, a, kind)) ->
    ...
    | (* Pick *) Coq_inr (Coq_inr (Coq_inl _)) -> collect_ps (k (Obj.magic (z_of_int (-50)))) (depth + 1)
    | (* Sched *) Coq_inr (Coq_inr (Coq_inr (Coq_inl tl))) ->
        List.fold_left (fun memset tid ->
          MemSet.union
            memset
    ...
    | (* void1 *) Coq_inr (Coq_inr (Coq_inr (Coq_inr (Coq_inr (Coq_inr _))))) -> MemSet.empty
